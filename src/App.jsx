...
const App = () => {
  ...
  return (
    <div className='h-screen flex flex-col'>
      <div className='flex flex-col justify-center text-center py-4 bg-sky-950 shadow-[0_4px_8px_rgba(0,0,0,0.5)] z-10'>
        <p className='text-3xl text-cyan-400 font-extrabold'>WaveX</p>
        <p className='text-sm text-cyan-600 font-extrabold'>By Amrendra Maurya</p>
      </div>
      <div className='main h-screen flex flex-row'>
        <div className='songsList w-96 bg-cyan-950 overflow-y-hidden p-8 z-5 shadow-[4px_0_8px_rgba(0,0,0,0.5)]'>
          <h1 className='text-white text-2xl bg-cyan-950 font-bold text-center p-4'>Songs List</h1>
          <ul className='flex flex-col gap-4'>
            {songs.map((song, index) => {
              return (
                <button key={index} className={`bg-cyan-700 cursor-pointer p-2 active:bg-cyan-900 rounded text-white hover:bg-cyan-800 shadow-[4px_2px_8px_rgba(0,0,0,0.4)]`} onClick={() => {
        ...
        <div className='player flex-1 overflow-hidden bg-gray-900 flex flex-col justify-center items-center gap-8'>
          <div className='artwork rounded w-80 h-80 md:w-64 md:h-64 bg-cyan-950 flex justify-center items-center'>
            <img src={artwork} draggable="false" />
          </div>
          ...
          <div className='buttons flex flex-row gap-8'>
            <button className='rounded-full active:bg-cyan-600 p-4 cursor-pointer text-cyan-400 text-4xl font-bold hover:bg-cyan-500 hover:text-black'
              onClick={handleLeft}
            >
              <FaAngleDoubleLeft />
            </button>
            <button className='rounded-full active:bg-cyan-600 p-6 cursor-pointer bg-cyan-400 text-black font-bold hover:bg-cyan-500' onClick={handleClick}>
              {playText ? <FaPause /> : <FaPlay />}
            </button>
            <button className='rounded-full active:bg-cyan-600 p-6 cursor-pointer bg-cyan-400 text-black font-bold hover:bg-cyan-500'
              onClick={handleStop}
            >
              <FaStop />
            </button>
